...
namespace GenGra
{
    public class GenGraGenerator : MonoBehaviour
    {
        ...
        {
            [SerializeField]
            private string missionSymbol;

            [SerializeField]
            private GameObject buildingInstructionsPrefab;
        ...
        [SerializeField]
        private string missionGraphGrammarFilePath;
         ...
         */
        [SerializeField]
        private BuildingInstructionsHolder[] buildingInstructionsByMissionSymbol;
        ...
        public GenGraType DeserializeGenGraXML()
        {
            using (var fileStream = new FileStream(missionGraphGrammarFilePath, FileMode.Open))
            {
                var serializer = new XmlSerializer(typeof(GenGraType));
        ...
        private void DebugLogGraph(GraphType graph)
        {
            ...
            for (var j = 0; j < graph.Edges.Edge.Length; j++)
            {
         ...
         */
        public IDictionary<string, GameObject[]> GenerateSpace(GraphType missionGraph)
        {
        ...
        private IDictionary<string, GameObject[]> BreadthFirstSpaceGeneration(GraphType graph, NodeType startNode)
        {
            IDictionary<string, GameObject[]> generatedSpace =
                new Dictionary<string, GameObject[]>(graph.Nodes.Node.Length);
